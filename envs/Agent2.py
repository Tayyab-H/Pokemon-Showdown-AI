...
class Agent:
    ...
    def login(self):
        ...
        string = '{"act":"login","name":"rlshowdowntrainer","pass":"throwawaypass",' + '"challstr":' + '"' + challstr + '"' + '}'
        js = json.loads(string)
        r = requests.post("http://play.pokemonshowdown.com/action.php?", js)
        r = r.content[1:]
        r = json.loads(r)
        assertion = r["assertion"]
        self.ws.send("|/trn rlshowdowntrainer,0," + assertion)
        print(self.ws.recv())
        print(self.ws.recv())
        print(self.ws.recv())
        print(self.ws.recv())
    ...
    def getResponse(self):
        while True:
            ...
            if "win|RLShowdownTrainer" in x:
                self.reward += 200
                self.isDone = True
            if "win|" in x and "RLShowdownTrainer" not in x:
                self.reward -= 100
                self.isDone = True
